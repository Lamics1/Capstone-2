...
import com.example.coach_booking_system.Service.PrivateSessionService;
...
public class PrivateSessionController {
    ...
    @PostMapping("/add")
    public ResponseEntity<?> add(@Valid @RequestBody PrivateSession privateSession, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity<?> update(@PathVariable Integer id, @Valid @RequestBody PrivateSession privateSession, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
