...
import com.example.coach_booking_system.Service.CoachReviewService;
...
public class CoachReviewController {
    ...
    // Add review
    @PostMapping("/add")
    public ResponseEntity<?> addReview(@Valid @RequestBody CoachReviews coachReview, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity<?> updateReview(@PathVariable Integer id, @Valid @RequestBody CoachReviews coachReview, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
